...
export default function Profile() {
  ...
  const topicData = Object.keys(topics).map((key) => ({
    subject: key,
    A: topics[key]?.rating || 1000, // Default to base rating if user hasn't played topic
    fullMark: 2000, // Max scale (adjustable)
  }));
  ...
  const handleLogout = () => {
    // Basic logout reuse from dashboard logic if needed locally,
    // though Navbar usually handles it.
    ...
    <div className="min-h-screen bg-[#0a0e27] text-white selection:bg-cyan-500/30 overflow-x-hidden relative pb-12">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 relative z-10 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-1 lg:col-span-1 bg-white/5 backdrop-blur-sm border border-white/10 rounded-3xl p-6 relative flex flex-col items-center">
          ...
          {/* Match History (2/3 width on LG, 1/2 on MD) */}
